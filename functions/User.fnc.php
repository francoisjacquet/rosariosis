...
{	global $_ROSARIO;
	...
	if(!$_ROSARIO['User'] || $_SESSION['UserSyear']!=$_ROSARIO['User'][1]['SYEAR'])
	{
		...
		{
            $sql = "SELECT STAFF_ID,USERNAME,FIRST_NAME||' '||LAST_NAME AS NAME,PROFILE,PROFILE_ID,SCHOOLS,CURRENT_SCHOOL_ID,EMAIL,SYEAR FROM STAFF WHERE SYEAR='".$_SESSION['UserSyear']."' AND USERNAME=(SELECT USERNAME FROM STAFF WHERE SYEAR='".Config('SYEAR')."' AND STAFF_ID='".$_SESSION['STAFF_ID']."')";
