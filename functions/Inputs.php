...
//modif Francois: add <label> on radio
		...
		if(count($options))
		{
			...
			{
				$key .= '';
				$table .= '<TD><label><INPUT type="radio" name="'.$name.'" value="'.str_replace("'",'&#39;',htmlspecialchars($key)).'" '.($value==$key && (!($value==false && $value!==$key) || ($value==='0' && $key===0))?'checked':'').' /><BR />'.str_replace("'",'&#39;',(is_array($val)?$val[0]:$val)).'</label></TD><TD>&nbsp;</TD>';
			}
		}
		$table .= '</TR></TABLE>';
