...
//modif Francois: remove ereg
		{
			...
			$toEscape = '<TEXTAREA id="textarea'.$name.'" name="'.$name.'" '.$options.'>'.preg_replace("/[\n\r]/",'\u000D\u000A',str_replace("\r\n",'\u000D\u000A',htmlspecialchars($value))).'</TEXTAREA>'.($title!=''?'<BR />'.(mb_strpos(mb_strtolower($title),'<span ')===false?'<span class="legend-gray">':'').'<label for="'.$name.'">'.$title.'</label>'.(mb_strpos(mb_strtolower($title),'<span ')===false?'</span>':'').'':'');
			$return = '<script type="text/javascript">var html'.str_replace(array('[',']'),'',$name).'=\''.$toEscape.'\';</script>'.$return;
...
//modif Francois: add <label> on radio
		...
		if(count($options))
		{
			...
			{
				$key .= '';
				$table .= '<TD><label><INPUT type="radio" name="'.$name.'" value="'.htmlspecialchars($key).'" '.($value==$key && (!($value==false && $value!==$key) || ($value==='0' && $key===0))?'checked':'').' /><BR />'.str_replace("'",'&#39;',(is_array($val)?$val[0]:$val)).'</label></TD><TD>&nbsp;</TD>';
			}
		}
		$table .= '</TR></TABLE>';
