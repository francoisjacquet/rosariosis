...
if ( User( 'PROFILE' ) === 'admin' ) :
	...
	$help['School_Setup/Schools.php&new_school=true'] = '<p>' . _help( '<i>Add a School</i> allows you to add a school to the system. Complete the school\'s information, and hit the "Save" button.' ) . '</p>

<p>' . _help( 'To switch to the new school, change the school pull-down menu in the left frame from your current school to the new one.' ) . '</p>';
...
<ul>
<li>' . _help( '"Pull-Down" fields create menus from which you can select one option. To create this type of field, click on "Pull-Down" and then add your options (one per line) in the "Pull-Down/Auto Pull-down/Coded Pull-Down/Select Multiple from Options" text box.' ) . '</li>
...
<li>' . _help( '"Edit Pull-Down" fields are similar to Auto Pull-Down fields.' ) . '</li>
<li>' . _help( '"Coded Pull-Down" fields are created by adding options to the large text box respecting the following pattern: "option shown"|"option stored in database" (where | is the "pipe" character). For example: "Two|2", where "Two" is displayed on screen to the user, or in a downloaded spreadsheet, and "2" is stored in the database.' ) . '</li>
<li>' . _help( '"Export Pull-Down" fields are created by adding options to the large text box respecting the same pattern used for "Coded Pull-Down" fields ("option shown"|"option stored in database"). For example: "Two|2", where "Two" is displayed on screen to the user, and "2" is the value in a downloaded spreadsheet, but "Two" is stored in the database.' ) . '</li>
