	...
	foreach($_REQUEST['tables'] as $id=>$columns)
	{
		...
		if(!isset($error) && $go)
		{
			...
			if ($table=='GRADEBOOK_ASSIGNMENTS')
			{
				...
				{
					//hook
					do_action('Grade/Assignments.php|create_assignment');
				}
				else
					//hook
					do_action('Grade/Assignments.php|update_assignment');
			}
		}
	}
		...
		if(!$_REQUEST['assignment_id'])
		{
			...
			if(count($assignments_RET))
			{
				foreach($assignments_RET as $assignment_id)
				{
					...
					//hook
					do_action('Grade/Assignments.php|delete_assignment');
...

{
	...
	{
		$delete_button = '<script>var delete_link = document.createElement("a"); delete_link.href = "Modules.php?modname='.$_REQUEST['modname'].'&modfunc=delete&assignment_type_id='.$_REQUEST['assignment_type_id'].'&assignment_id='.$_REQUEST['assignment_id'].'"; delete_link.target = "body";</script>';
