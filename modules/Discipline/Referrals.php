		...
		switch($category['DATA_TYPE'])
		{
			...
			case 'multiple_radio':
				if(AllowEdit() && !isset($_REQUEST['_ROSARIO_PDF']))
				{
					...
					$toEscape .= '</TR></TABLE>';
					echo str_replace('"','\"',$toEscape);
					echo '","divvalues[CATEGORY_'.$category['ID'].']'.'",true);\' >'.'<span class="underline-dots">'.(($RET['CATEGORY_'.$category['ID']]!='')?str_replace('"','&rdquo;',$RET['CATEGORY_'.$category['ID']]):'-').'</span>'."</div></DIV>";
				}
				else
