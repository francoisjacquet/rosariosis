...
if($_REQUEST['table']=='')
	$_REQUEST['table'] = '0';
$category_select = "<SELECT name=table onChange='this.form.submit();'><OPTION value='0'".($_REQUEST['table']=='0'?' SELECTED="SELECTED"':'').">"._('Attendance')."</OPTION>";
	...
	foreach($RET as $staff_id=>$periods)
	{
		...
		foreach($periods as $period)
		{
			if(!isset($_REQUEST['_ROSARIO_PDF']))
				$staff_RET[$i][$period['PERIOD_ID']] .= button($period['COMPLETED']=='Y'?'check':'x','','"#" onMouseOver=\'stm(["'._('Course Title').'","'.str_replace("'",'&#39;',$period['COURSE_TITLE']).'"],tipmessageStyle);\' onMouseOut=\'htm()\'').' ';
			else
...
else
{
	...
	foreach($RET as $staff_id=>$periods)
	{
		...
		{
			if(!isset($_REQUEST['_ROSARIO_PDF']))
				$RET[$staff_id][$id]['COMPLETED'] = button($period['COMPLETED']=='Y'?'check':'x','','"#" onMouseOver=\'stm(["'._('Course Title').'","'.str_replace("'",'&#39;',$period['COURSE_TITLE']).'"],tipmessageStyle);\' onMouseOut=\'htm()\'').' ';
			else
