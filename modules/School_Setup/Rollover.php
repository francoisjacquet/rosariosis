...
foreach ( (array) $tables as $table => $name)
{
	if ( $table!='FOOD_SERVICE_STAFF_ACCOUNTS')
		$exists_RET[ $table ] = DBGet( "SELECT count(*) AS COUNT FROM $table WHERE SYEAR='".$next_syear."'".(! $no_school_tables[ $table ]?" AND SCHOOL_ID='".UserSchool()."'":'') );
	else
		$exists_RET['FOOD_SERVICE_STAFF_ACCOUNTS'] = DBGet( "SELECT count(*) AS COUNT FROM STAFF WHERE SYEAR='".$next_syear."' AND exists(SELECT * FROM FOOD_SERVICE_STAFF_ACCOUNTS WHERE STAFF_ID=STAFF.STAFF_ID)" );
...
{	global $next_syear,$RosarioModules;
	...
	switch ( $table)
	{
		...
		case 'SCHOOL_MARKING_PERIODS':
			...
			$mp_next = DBGet( "SELECT MARKING_PERIOD_ID,ROLLOVER_ID,MP FROM SCHOOL_MARKING_PERIODS WHERE (MP='QTR' OR MP='SEM') AND SYEAR='".$next_syear."' AND SCHOOL_ID='".UserSchool()."' ORDER BY SORT_ORDER" );
		...
		case 'COURSES':
			...
			// ROLL COURSES
			DBQuery("INSERT INTO COURSES (SYEAR,COURSE_ID,SUBJECT_ID,SCHOOL_ID,GRADE_LEVEL,TITLE,SHORT_NAME,CREDIT_HOURS,DESCRIPTION,ROLLOVER_ID) SELECT SYEAR+1,".db_seq_nextval('COURSES_SEQ').",(SELECT SUBJECT_ID FROM COURSE_SUBJECTS s WHERE s.SCHOOL_ID=c.SCHOOL_ID AND s.ROLLOVER_ID=c.SUBJECT_ID),SCHOOL_ID,GRADE_LEVEL,TITLE,SHORT_NAME,CREDIT_HOURS,DESCRIPTION,COURSE_ID FROM COURSES c WHERE SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."'");
			...
			$categories_RET = DBGet( "SELECT ID,ROLLOVER_ID FROM ATTENDANCE_CODE_CATEGORIES WHERE SYEAR='".$next_syear."' AND SCHOOL_ID='".UserSchool()."' AND ROLLOVER_ID IS NOT NULL" );
		...
		case 'REPORT_CARD_GRADES':
			...
			DBQuery("INSERT INTO REPORT_CARD_GRADE_SCALES (ID,SYEAR,SCHOOL_ID,TITLE,COMMENT,HR_GPA_VALUE,HHR_GPA_VALUE,SORT_ORDER,ROLLOVER_ID,GP_SCALE,HRS_GPA_VALUE,GP_PASSING_VALUE) SELECT ".db_seq_nextval('REPORT_CARD_GRADE_SCALES_SEQ').",SYEAR+1,SCHOOL_ID,TITLE,COMMENT,HR_GPA_VALUE,HHR_GPA_VALUE,SORT_ORDER,ID,GP_SCALE,HRS_GPA_VALUE,GP_PASSING_VALUE FROM REPORT_CARD_GRADE_SCALES WHERE SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."'");
