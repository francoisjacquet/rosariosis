		...
		if (is_numeric($_REQUEST['amount']))
		{
			...
			foreach($_REQUEST['student'] as $student_id=>$yes)
			{
					$sql = "INSERT INTO BILLING_FEES (STUDENT_ID,ID,TITLE,AMOUNT,SYEAR,SCHOOL_ID,ASSIGNED_DATE,DUE_DATE,COMMENTS)
								values('".$student_id."',".db_seq_nextval('BILLING_FEES_SEQ').",'".str_replace("\'","''",$_REQUEST['title'])."','".preg_replace('/[^0-9,.]+/','',$_REQUEST['amount'])."','".UserSyear()."','".UserSchool()."','".DBDate()."','".$due_date."','".str_replace("\'","''",$_REQUEST['comments'])."')";
					DBQuery($sql);
			}
