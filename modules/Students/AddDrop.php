...
$enrollment_RET = DBGet(DBQuery("SELECT se.START_DATE AS START_DATE,NULL AS END_DATE,se.START_DATE AS DATE,se.SCHOOL_ID,se.STUDENT_ID,s.LAST_NAME||', '||s.FIRST_NAME AS FULL_NAME 
FROM STUDENT_ENROLLMENT se,STUDENTS s 
WHERE s.STUDENT_ID=se.STUDENT_ID 
AND se.START_DATE BETWEEN '".$start_date."' AND '".$end_date."'
UNION 
SELECT NULL AS START_DATE,se.END_DATE AS END_DATE,se.END_DATE AS DATE,se.SCHOOL_ID,se.STUDENT_ID,s.LAST_NAME||', '||s.FIRST_NAME AS FULL_NAME 
FROM STUDENT_ENROLLMENT se,STUDENTS s 
WHERE s.STUDENT_ID=se.STUDENT_ID 
...
$columns = array('FULL_NAME'=>_('Student'),'STUDENT_ID'=>sprintf(_('%s ID'),Config('NAME')),'SCHOOL_ID'=>_('School'),'START_DATE'=>_('Enrolled'),'END_DATE'=>_('Dropped'));
