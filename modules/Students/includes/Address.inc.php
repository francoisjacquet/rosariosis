		...
		if(count($addresses_RET))
		{
			foreach($addresses_RET as $address_id=>$addresses)
			{
				...
				if(count($xstudents))
				{
					...
					echo '<TH>'.button('warning','','"#" onMouseOver=\'stm(["'._('Warning').'","'.str_replace('"','\"',$warning).'"],tipmessageStyle);\' onMouseOut=\'htm()\'').'</TH>';
				}
				else
					echo '<TH>&nbsp;</TH>';
				}
...
//modif Francois: css WPadmin
				...
				if($_REQUEST['person_id']!='new')
				{
					...
					echo '<TR><TD>'._makeAutoSelectInputX($this_contact['STUDENT_RELATION'],'STUDENT_RELATION','STUDENTS_JOIN_PEOPLE',_('Relation'),array()).'</TD>';
