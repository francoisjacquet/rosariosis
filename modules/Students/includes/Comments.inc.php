...
if($_REQUEST['modfunc']=='update' && AllowEdit())
{
	...
	$_REQUEST['values']['STUDENT_MP_COMMENTS'][UserStudentID()]['COMMENT'] = date('Y-m-d G:i:s').'|'.User('STAFF_ID')."||".pg_escape_string($_REQUEST['values']['STUDENT_MP_COMMENTS'][UserStudentID()]['COMMENT']);
	...
	if(!$existing_RET)
		...
		$_REQUEST['values']['STUDENT_MP_COMMENTS'][UserStudentID()]['COMMENT'] = pg_escape_string($existing_RET[1]['COMMENT'])."||".$_REQUEST['values']['STUDENT_MP_COMMENTS'][UserStudentID()]['COMMENT'];
