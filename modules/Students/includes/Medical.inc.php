...
if(empty($_REQUEST['modfunc']))
{
	...
	$link['remove']['link'] = 'Modules.php?modname='.$_REQUEST['modname'].'&include='.$_REQUEST['include'].'&modfunc=delete&table=STUDENT_MEDICAL&title='.urlencode(_('Immunization or Physical'));
	...
	$link['remove']['link'] = 'Modules.php?modname='.$_REQUEST['modname'].'&include='.$_REQUEST['include'].'&modfunc=delete&table=STUDENT_MEDICAL_ALERTS&title='.urlencode(_('Medical Alert'));
	$link['remove']['variables'] = array('id'=>'ID');
	...
	if(User('PROFILE')=='admin' || User('PROFILE')=='teacher')
	{
		...
		$link['remove']['link'] = 'Modules.php?modname='.$_REQUEST['modname'].'&include='.$_REQUEST['include'].'&modfunc=delete&table=STUDENT_MEDICAL_VISITS&title='.urlencode(_('Nurse Visit'));
		$link['remove']['variables'] = array('id'=>'ID');
