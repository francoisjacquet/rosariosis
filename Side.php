...
if(UserStudentID() && (User('PROFILE')=='admin' || User('PROFILE')=='teacher'))
{
	...
	echo '<TABLE class="width-100p cellspacing-0 cellpadding-0" style="background-color:#333366;"><TR><TD><A HREF="Side.php?student_id=new&modname='.$_REQUEST['modname'].'" target="menu"><IMG SRC="assets/x_button.png" height="24" style="vertical-align: middle;"></A></TD><TD><B>'.(AllowUse('Students/Student.php')?'<A HREF="Modules.php?modname=Students/Student.php&student_id='.UserStudentID().'">':'').'<span style="color:white" class="size-2">'.$RET[1]['FULL_NAME'].'</span>'.(AllowUse('Students/Student.php')?'</A>':'').'</B></TD></TR></TABLE>';
}
	...
	if(count($_ROSARIO['Menu'][$modcat]))
	{
		...
		foreach($keys as $key_index=>$file)
		{
			...
			elseif(!is_numeric($file))
				echo '<TR><TD><A HREF="Modules.php?modname='.$file.'" onclick="var modname=\''.$file.'\'; selMenuA(modname);"'.(mb_stripos($file,'_ROSARIO_PDF') !== false ? ' target="_blank"' : '').'>'.$title.'</A></TD></TR>';
